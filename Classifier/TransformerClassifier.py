...
class ClassificationHead(nn.Module):
    def __init__(self, config, embed_dim, kernel_num=3, kernel_sizes=[2, 3, 4, 5], num_labels=2):
        ...
        self.conv_layers = nn.ModuleList([nn.Conv2d(1, self.kernel_num, (k, self.embed_dim)) for k in self.kernel_sizes])
...
if __name__ == "__main__":
    ...
    tokenizer = RobertaTokenizer(tokenizer_root + "/tokenizer/aster-vocab.json",
                                 tokenizer_root + "/tokenizer/aster-merges.txt")
    ...
    dataset = BugDataset(project_name=project_name, scratch_path=scratch_path, dataframe=combined_df)
    config = AutoConfig.from_pretrained(model_path,
                                        num_labels=1)  # RobertaConfig.from_pretrained(model_path, num_labels=1)
    model = AutoModelForSequenceClassification.from_pretrained(model_path, config=config).to(
        dev)  # RobertaForSequenceClassification.from_pretrained(model_path, config=config)
    model = freeze_model(model, model_name)

    Path(root_path + "_Dataset/{}/".format(project_name.split("/")[-1])).mkdir(parents=True, exist_ok=True)
    pickle.dump(dataset, open(
        root_path + "_Dataset/{}/{}_full_dataset.pickle".format(project_name.split("/")[-1],
    ...
    dataloader = DataLoader(dataset, batch_size=batch_size, pin_memory=True, num_workers=2, sampler=sampler,
                            drop_last=True)
    criterion = nn.BCELoss()
    optimizer = optim.Adam(model.parameters(), lr=0.01)
    loss_list = []

    Path(root_path + "_Model").mkdir(parents=True, exist_ok=True)
    ...
    for epoch in range(1, 5):  # loop over the dataset multiple times
        ...
        torch.save(model, root_path + "_Model/Model_{}".format(epoch + 1))
